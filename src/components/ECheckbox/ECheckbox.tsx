...
type ECheckbox = ComponentConfiguration<
...
const ECheckbox = defineComponent({
  ...
  inheritAttrs: false,
  setup(_, { emit }) {
    const component = useComponent<ECheckbox>(configuration, _);
    ...
    render(() => (
      <EFormInput
        feedback={!!props.value.feedback || slot.active('feedback')}
          ...
          <div>
            ...
            <div class={css.element('target')}>
              {props.value.indeterminate ? (
                <EIcon
                  icon={props.value.iconIndeterminate}
